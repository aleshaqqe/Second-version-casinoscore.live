...
function generateDreamCatcherExtraStats(game, spinData) {
  const trackedSpins = spinData.slice(0, 39);
  const totalSpins = trackedSpins.length;
  const multiplierKeys = ["2x", "7x"];

  const multiplierRows = multiplierKeys
    .map((key) => {
  ...
  const totalMultiplierHits = multiplierRows.reduce((sum, row) => sum + row.hits, 0);

  multiplierRows.forEach((row) => {
    row.sharePercent = totalMultiplierHits
      ? Number(((row.hits / totalMultiplierHits) * 100).toFixed(2))
      : 0;
  });
